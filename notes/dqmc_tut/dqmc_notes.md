...
If only $H_0$ is non-zero, one can directly diagonalize the one-body matrix $T = U \lambda U^\dagger$ to get well-defined energy levels. The systems are just well-defined single particles, and the computaitonal complexity to diagonalize this $N\times N$ matrix is $O(N^3)$.

$$
    H_0 = \sum_{ij} T_{ij}c_i^\dagger c_j = \sum_{ijk} (c_i^\dagger U_{ik}) \lambda_k  (U_{kj}^\dagger c_j) = \sum_k \lambda_k d_k^\dagger d_k
$$
...
Suppose, the interaction term $\hat H_I = \sum_i \lambda_i \hat A_i^2$ can be written in terms of a summation of squares of non-interacting operator $\hat A_i = c^\dag V_i c$, its propagator with propagation length $\Delta\tau$ can also be represented using the stochastic summation (explain what is a propagator)
...
At the single-particle level, since $\hat H_0 = c^\dagger T c$ and $\hat A_i = c^\dagger V_i c$ are both fermion bilinears, each short-time factor is represented by an $N\times N$ matrix. For one imaginary-time slice $t$, define

$$
\begin{aligned}
    B_t
    &= e^{-\Delta\tau T}
    \prod_i e^{\eta_{l_{t,i}}\sqrt{-\lambda_i\Delta\tau}\,V_i}, \\
    l_t &= \{l_{t,i}\}_i
\end{aligned}
$$

and for the full auxiliary-field history $\mathcal{C} = \{l_{t,i}\}$ define
...
If we keep the Gauss--Hermite coefficients explicitly, they only contribute a scalar prefactor $\prod_{t,i}\gamma_{l_{t,i}}$. In particular, the finite-temperature weight can be written as

$$
    W[\mathcal{C}]
    = \text{Tr}\, U_\mathcal{C}
    = \left(\prod_{t,i}\gamma_{l_{t,i}}\right)\det[1 + B_\mathcal{C}]
$$
...
$$
    ...
    = \left(\prod_{t,i}\gamma_{l_{t,i}}\right)\det[P^\dagger B_\mathcal{C} P]
$$

This equation looks almost the same as Ising model, except we don't konw if the weight $W[\mathcal{C}]$ is nonnegative or not. If $W[\mathcal{C}]$ is nonnegative, we can build an exactly same MCMC algorithm comparing to Ising model.

Since $U_\mathcal{C}$ represents a squence of $N_\tau$ non-interacting propagators, the complexity for computing $W[\mathcal{C}]$ is $O(N_\tau N^3)$.
...
## Practical implementation [work in progress]
In practice, one usually starts from a random configuration $\mathcal{C}$ and then do local update sweeps 
```
for t = 1, ..., Ntau
    for i = 1, ..., N
        propose flip of l[t, i]
        acc/rej
```
It would be too expensive if one recompute $W[\mathcal{C}]$ during each local update.

## Minimal implementation
An minimal DQMC program for 1D/2D Hubbard model written in Julia are under [hubbard-afqmc](https://github.com/haoran0115/haoran0115.github.io/tree/main/notes/dqmc_tut/hubbard-afqmc) directory
...
electrons](https://pawn.physik.uni-wuerzburg.de/~assaad/Reprints/assaad_evertz.pdf): part of this tutorial and most of the minimal implementation is mainly based on the second half (DQMC part) of this lecture notes
